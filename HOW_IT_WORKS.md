...
Power on
  ...
  → 100 Hz game loop
       ...
       → win when within ~3 ft (0.9 m)  (must exceed one stride, see navigation.c)
...
| `WIN_RADIUS_M` | 0.90 m (~3 ft) | How close to win (must exceed `STRIDE_M`) |
...
sensing range, 1.0 when at the 0.9 m win threshold.
